...
// The character class is intentionally ASCII-only and case-sensitive: matching
// case-insensitively under /u would also accept the Unicode case-folding
// look-alikes (U+212A KELVIN SIGN, U+017F LATIN SMALL LETTER LONG S).
...
function parseWithStatus (uri, opts) {
  ...
  if (matches) {
    ...
    // Decode and validate the scheme up front: percent-encoding hides
    // delimiters ("%2f%2f" -> "//") and control characters ("%0d%0a" -> CRLF)
    // from the parser, and decoding them later, after the authority has
    // already been resolved, lets normalization synthesize an authority or
    // smuggle a header break into the serialized output. A scheme that does
    // not decode to a valid RFC 3986 scheme is rejected, never rewritten.
    ...
    if (!options.unicodeSupport && (!schemeHandler || !schemeHandler.unicodeSupport)) {
      ...
      // A malformed IP literal is never handed to the WHATWG URL parser: it is
      // not a domain name, and some implementations "repair" it into a
      // different, valid host (e.g. dropping an invalid zone identifier so
      // "[fe80::1%25eth 0]" becomes "[fe80::1]"), which is exactly the silent
      // rewrite this rejection is meant to prevent.
...
function normalizeComparableURI (uri, opts) {
  ...
  if (typeof uri === 'object') {
    // A component object carrying a scheme that decodes to something other
